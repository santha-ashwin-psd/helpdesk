    ...
    >
      ...
      >
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <UniInput
            v-if="!customerInTemplate && !isCustomerPortal"
            :field="{ fieldname: 'customer', label: __('Customer'), fieldtype: 'Link', options: 'HD Customer' }"
            :value="customer"
            @change="(e) => customer = e.value"
          />
          <div v-if="!machineInTemplate" class="flex flex-col gap-2">
            <span class="block text-sm text-ink-gray-6">
              {{ __("Machine") }}
            </span>
            <FormControl
          ...
          <div v-if="!locationInTemplate" class="flex flex-col gap-2">
            <span class="block text-sm text-ink-gray-6">
              {{ __("Location") }}
            </span>
            <FormControl
        ...
        <UniInput
          v-if="!priorityInTemplate && !isCustomerPortal"
          :field="{ fieldname: 'priority', label: __('Priority'), fieldtype: 'Link', options: 'HD Ticket Priority' }"
          :value="priority"
          @change="(e) => priority = e.value"
        />
        <SearchArticles
...
// Treat any template field with the same fieldname as owning that field (include Link fields)
const customerInTemplate = computed(() =>
  visibleFields.value?.some((f) => f.fieldname === "customer")
);
const machineInTemplate = computed(() =>
  visibleFields.value?.some((f) => f.fieldname === "machine")
);
const locationInTemplate = computed(() =>
  visibleFields.value?.some((f) => f.fieldname === "location")
);
const priorityInTemplate = computed(() =>
  visibleFields.value?.some((f) => f.fieldname === "priority")
);
...
const ticket = createResource({
  ...
  makeParams: () => ({
    doc: {
      ...
      ...templateFields,
      // If template owns these fields, prefer templateFields values so we don't overwrite them with empty refs
      customer: customerInTemplate.value ? templateFields.customer : customer.value,
      machine: machineInTemplate.value ? templateFields.machine : machine.value,
      location: locationInTemplate.value ? templateFields.location : location.value,
      priority: priorityInTemplate.value ? templateFields.priority : priority.value,
    },
