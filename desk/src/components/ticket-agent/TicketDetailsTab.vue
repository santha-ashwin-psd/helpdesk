      ...
      <div class="mt-4">
        ...
        >
          <template v-for="field in section.fields">
            <FormControl
              v-if="field.visible && ['machine','location'].includes(field.fieldname)"
              :key="field.fieldname"
              ...
              @update:model-value="(val) => handleFieldUpdate(field.fieldname, val, true)"
              type="text"
            />
            <Link
...
const coreFields = computed(() => {
  ...
  const _coreFields = [
    ...
    { group: true, fields: [getField("customer"), getField("machine")] },
    { group: false, fields: [getField("location")] },
